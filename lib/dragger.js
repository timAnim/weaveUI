...
var Dragger = {
    ...
    dragStart: function (ev) {
        ...
        Dragger.boundery = [Dragger.outer.getBoundingClientRect().top, Dragger.outer.getBoundingClientRect().right, Dragger.outer.getBoundingClientRect().bottom, Dragger.outer.getBoundingClientRect().left];
        if (Dragger.isM) {
...
var Resizer = {
    ...
    start: function (ev) {
        ...
        if (Resizer.data.onStart) {
            Resizer.data.onStart(Resizer.origin);
        }
        if (Resizer.seed === 0) {
            Resizer.seed = setInterval(function () {
                Resizer.data.onDrag(Resizer.delta);
            }, 16);
        }
    },
    move: function (ev) {
    ...
    init: function (callback) {
        ...
        var list,
            ...
            if (sessionStorage.isMobile == 'true') {
                if (useScroll) canvas.style.overflow = 'auto';else canvas.style.overflow = 'hidden';
            }
